# Milestone 28 sixth enterprise census

This sanitized three-run report records the frozen Milestone 22 replay after
CN5 static `Integer.valueOf` support. The manifest hash is
...
strictly match. Cold/warm/warm durations were 237,313 ms, 241 ms, and 99 ms.
The first blocker is generated custom-share SObject resolution for
`Log__Share` in `LogHandler.cls`, affecting 1,121 tests; `Flow.Interview`
affects 18 tests. No selected next-family implementation has started.
...
cargo +1.88.0 run --release --locked -- enterprise run \
  ...
  --output evidence/milestone28/census-6/report.json
```
