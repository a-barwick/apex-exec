# Milestone 28 eighth enterprise census

This sanitized three-run report records the frozen Milestone 22 replay after
CN7 organization-limit support. The manifest hash is
...
strictly match. Cold/warm/warm durations were 228,558 ms, 73 ms, and 73 ms.
CN7 removed `System.OrgLimit` and `System.OrgLimits.getMap()` as the first
blocker. The new first blocker is the missing `User.IsActive` standard-schema
...
cargo run --release --locked -- enterprise run \
  ...
  --output evidence/milestone28/census-8/report.json
```
