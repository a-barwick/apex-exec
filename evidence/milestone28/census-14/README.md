# Milestone 28 fourteenth enterprise census

This sanitized three-run report records the frozen Milestone 22 replay after
CN13 `Approval.LockResult` support. The manifest hash is
...
strictly match. Cold/warm/warm durations were 401,814 ms, 90 ms, and 91 ms.
CN13 removed the missing `Approval.LockResult` diagnostic. The new first
blocker is missing `Approval.ProcessResult` in `LogEntryEventBuilder.cls`,
affecting 1,005 tests. The remaining first-error families affect 32 tests or
fewer.
...
cargo +1.88.0 run --release --locked -- enterprise run \
  ...
  --output evidence/milestone28/census-14/report.json
```
