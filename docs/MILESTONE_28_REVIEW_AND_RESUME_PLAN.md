...
| M28-Q2 | Standard `User` schema restoration | Complete (`666eebd`; reviewed and integrated; format/check pass, M27 suite 10/10, standard-schema units 2/2) | Q1 integrated at `7d706c9` | Standard schema, M27 tests |
...
| M28-CN7 | One next-ranked compatibility family | Complete (integrated at `ac756a0`; independent review approved; typed `System.OrgLimit`, one-snapshot `System.OrgLimits.getMap`, focused Salesforce 2/2, full Rust/Clippy/docs/Lizard/replay gates pass) | CENSUS-7 complete | Typed organization-limit platform values and host boundary |
...
local/Salesforce 2/2 evidence. Its sixth sealed census selected generated
custom-share SObject resolution for `Log__Share` (1,121 affected tests).
CN6 passed independent review and was integrated at `798c438`. Its seventh
sealed census remains 0/1,159 and selects `System.OrgLimit` plus
`System.OrgLimits.getMap` (1,121 affected tests) as CN7. CN7 passed independent
review and was integrated at `ac756a0` with one explicit host snapshot per map
request, typed name/value/limit accessors, focused local/Salesforce evidence
matching 2/2 dimensions, and guarded fixture cleanup. Its eighth sealed census
is recorded at `evidence/milestone28/census-8/report.json`; strict compatibility
remains 0/1,159, and the same 1,121 tests now stop at the missing
`User.IsActive` Boolean standard-schema field. CN8 passed independent review
...
`FlowDefinitionView.IsActive`. CN10 passed independent review and was
...
`Messaging.SingleEmailMessage`. CN12 passed independent review after bounded
corrections and was integrated at `b88a8f2`. Its thirteenth sealed census is
recorded at `evidence/milestone28/census-13/report.json`; strict compatibility
remains 0/1,159, and 1,005 tests now stop at missing
`Approval.LockResult`. CN13 passed independent review after bounded generic
placement corrections and was integrated at `95ea6db`. Its fourteenth sealed
census is recorded at `evidence/milestone28/census-14/report.json`; strict
compatibility remains 0/1,159, and the same 1,005 tests now stop at missing
`Approval.ProcessResult`. CN14 is now in Review on
`codex/m28-cn14-approval-process-result`. It models only scalar/direct-List
ProcessResult values, Salesforce-shaped JSON defaults and field order, the
Nebula accessors, typed errors including `NO_APPLICABLE_PROCESS`, and bounded
conversion/serialization. Its guarded API 65.0 comparison matches 2/2 without
approval or DML mutation, and the temporary class was deleted and verified
absent. Do not integrate CN14, mark it Complete, or start
`Approval.UnlockResult`, `Flow.Interview`, or another later family from this
task branch.

Its unchanged three-rerun candidate census preserves the frozen hashes and
1,159 denominator, discovers/parses 1,159 tests, and remains 0/1,159 strict
