...
- M27 is in implementation review. Checked class metadata now distinguishes
  ...
  unchanged API 66.0 oracle fixture passes locally. Verification covers 452
  Rust tests with no failures or ignored tests; LLVM source-line coverage is
  **84.92%** overall (30,749/36,208) and **84.02%** across 26 instrumented
  changed production Rust modules (18,737/22,300). `src/platform/mod.rs` has
  ...
  documentation, local CLI, and coverage checks pass. Guarded Salesforce
  capture remains an exit blocker, and the pinned Lizard gate could not fetch
  its tool after the approval service rejected escalation, so M27 is not yet
  complete.

## Immediate target

M26 metadata accounting and org-configuration breadth is complete. M27 sharing
and security profiles is in review pending guarded Salesforce evidence. The
